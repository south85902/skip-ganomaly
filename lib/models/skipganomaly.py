...
class Skipganomaly(BaseModel):
    ...
    def __init__(self, opt, data=None):
        ...
        if self.opt.netg == 'CAE':
            ...
            self.netg = define_G_DFR(self.opt, norm='batch', use_dropout=False, init_type='normal', kernel_size=self.ks)
        else:
            self.netg = define_G(self.opt, norm='batch', use_dropout=False, init_type='normal', kernel_size=self.ks)

        if self.opt.netg == 'Unet_DFR':
            self.netd = define_D_DFR(self.opt, norm='batch', use_sigmoid=False, init_type='normal', kernel_size=self.ks)
        else:
            self.netd = define_D(self.opt, norm='batch', use_sigmoid=False, init_type='normal', kernel_size=self.ks)
