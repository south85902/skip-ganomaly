...
class BasicDiscriminator_DFR(nn.Module):
    """
    NETD
    """
    def __init__(self, opt):
...
class BasicDiscriminator_WGAN(nn.Module):
    """
    NETD
    """
    def __init__(self, opt):
...
##
def define_G_DFR(opt, norm='batch', use_dropout=False, init_type='normal'):
    netG = None
    norm_layer = get_norm_layer(norm_type=norm)
    num_layer = int(np.log2(opt.isize))
...
##
def define_G_noSkipConnection(opt, norm='batch', use_dropout=False, init_type='normal'):
    netG = None
    norm_layer = get_norm_layer(norm_type=norm)
    num_layer = int(np.log2(opt.isize))
    netG = UnetGenerator_noSkipConnection(opt.nc, opt.nc, num_layer, opt.ngf, norm_layer=norm_layer, use_dropout=use_dropout, opt=opt)
    return init_net(netG, init_type, opt.gpu_ids)
...
##
def define_G_noSkipConnection_res(opt, norm='batch', use_dropout=False, init_type='normal'):
    netG = None
    norm_layer = get_norm_layer(norm_type=norm)
    num_layer = int(np.log2(opt.isize))
    netG = UnetGenerator_noSkipConnection_res(opt.nc, opt.nc, num_layer, opt.ngf, norm_layer=norm_layer, use_dropout=use_dropout, opt=opt)
    return init_net(netG, init_type, opt.gpu_ids)
...
def define_D_DFR(opt, norm='batch', use_sigmoid=False, init_type='normal'):
    netD = None
    norm_layer = get_norm_layer(norm_type=norm)
    netD = BasicDiscriminator_DFR(opt)
    return init_net(netD, init_type, opt.gpu_ids)

##
def define_W(opt, norm='batch', use_sigmoid=False, init_type='normal'):
    netD = None
    norm_layer = get_norm_layer(norm_type=norm)
    netD = BasicDiscriminator_WGAN(opt)
    return init_net(netD, init_type, opt.gpu_ids)
##############################################################################
# Classes
##############################################################################
...
class UnetGenerator_noSkipConnection(nn.Module):
    def __init__(self, input_nc, output_nc, num_downs, ngf=64,
                 norm_layer=nn.BatchNorm2d, use_dropout=False, opt=None):
        super(UnetGenerator_noSkipConnection, self).__init__()
...
class UnetGenerator_noSkipConnection_res(nn.Module):
    def __init__(self, input_nc, output_nc, num_downs, ngf=64,
                 norm_layer=nn.BatchNorm2d, use_dropout=False, opt=None):
        super(UnetGenerator_noSkipConnection_res, self).__init__()
        ...
        unet_block = UnetSkipConnectionBlock_noSkipConnection_res(output_nc, ngf, input_nc=input_nc, submodule=unet_block, outermost=True, norm_layer=norm_layer, opt=opt)
...
class UnetSkipConnectionBlock_noSkipConnection(nn.Module):
    def __init__(self, outer_nc, inner_nc, input_nc=None,
                 submodule=None, outermost=False, innermost=False, norm_layer=nn.BatchNorm2d, use_dropout=False, opt=None):
        super(UnetSkipConnectionBlock_noSkipConnection, self).__init__()
        self.outermost = outermost
        kernel_size = opt.ks
        if type(norm_layer) == functools.partial:
            use_bias = norm_layer.func == nn.InstanceNorm2d
